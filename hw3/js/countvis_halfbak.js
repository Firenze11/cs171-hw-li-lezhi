...
CountVis.prototype.updateVis = function(){
    ...
    // updates axis
    this.svg.select(".x.axis").call(this.xAxis);  //!!!!!!!!!!!!!!!!!!!!!!!!!!!call
...
CountVis.prototype.addSlider = function(svg){
    ...
    var sliderScale=d3.scale.linear()
                            .domain(d3.extent(this.displayData,function(d){return d.count;}))
                            .range([this.height,0]) ;
    var sliderDragged = function(){
        var value = Math.max(0, Math.min(200,d3.event.y));  /////////////////////200???
        var sliderValue = sliderScale.invert(value);
        // TODO: do something here to deform the y scale
        console.log("Y Axis Slider value: ", sliderValue);
        d3.select(this)
          .attr("y", function () { return sliderScale(sliderValue); })
        that.updateVis({});
    }
